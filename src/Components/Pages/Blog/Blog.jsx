            ...
            <div>
                <div className='flex flex-col bg-gray-100 p-5 mb-5 rounded-md'>
                    <p onClick={() => getContent('quesOne', 'quesOneAnswer')}>
                        <AiFillPushpin className={`${blogs.find((blog) => blog.question === 'What is an access token and refresh token? How do they work and where should we store them on the client-side?') ? 'text-blue-700' : ''} cursor-pointer`}></AiFillPushpin>
                    </p>
                ...
                <div className='flex flex-col bg-gray-100 p-5 mb-5 rounded-md'>
                    <p onClick={() => getContent('quesTwo', 'quesTwoAnswer')}>
                        <AiFillPushpin className={`${blogs.find((blog) => blog.question === 'Compare SQL and NoSQL database?') ? 'text-blue-700 cursor-pointer' : ''} cursor-pointer`}></AiFillPushpin>
                    </p>
                    <h1 id='quesTwo' className='font-roboto font-bold text-3xl'>Compare SQL and NoSQL database?</h1>

                    <p id='quesTwoAnswer' className='font-roboto text-xl'>
                        Sql stands form Standard Query language.Sql is the standard language form dealing with relational database.Sql database is a collection of highly structured tables.Relational database are built using the structured query language to create,store,update and retrive data

                        NoSQL database are not Structured.NoSql data based are document based,key-value,graph.NoSql database stores data in a collection.Collection can be a array.NoSql database structure are flexible anyone can store large amount of data and can modify data easily</p>
                </div>
                <div className='flex flex-col bg-gray-100 p-5 mb-5 rounded-md'>
                    <p onClick={() => getContent('quesThree', 'quesThreeAnswer')}>
                        <AiFillPushpin className={`${blogs.find((blog) => blog.question === 'What is express js? What is Nest JS?') ? 'text-blue-700' : ''} cursor-pointer`}></AiFillPushpin>
                    </p>
                    <h1 id='quesThree' className='font-roboto font-bold text-3xl'>What is express js? What is Nest JS?</h1>
                    <p id='quesThreeAnswer' className='font-roboto text-xl'>Express.js is a node.js web application framework which is used for building web-application and mobile applications using Node.js approaches and principles of Node event-driven architecture.Express js is used to build single page multipage web application.Express js is focus on server side devlopment

                        Next.js is a react framework for web.Next.js opens new door to create full-stack Web applications by extending the latest React features.Next.js integrating powerful Rust-based JavaScript tooling for the fastest builds.
                    </p>

                </div>
                <div className='flex flex-col bg-gray-100 p-5 mb-5 rounded-md'>
                    <p onClick={() => getContent('quesFour', 'quesFourAnswer')}>
                        <AiFillPushpin className={`${blogs.find((blog) => blog.question === 'What is MongoDB aggregate and how does it work?') ? 'text-blue-700' : ''} cursor-pointer`}></AiFillPushpin>
                    </p>
                    <h1 id='quesFour' className='font-roboto text-3xl font-bold'>What is MongoDB aggregate and how does it work?</h1>
